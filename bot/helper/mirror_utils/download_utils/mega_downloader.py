...
from bot import LOGGER, MEGA_API_KEY, download_dict_lock, download_dict, MEGA_EMAIL_ID, MEGA_PASSWORD, MEGA_LIMIT, STOP_DUPLICATE, ZIP_UNZIP_LIMIT, TAR_UNZIP_LIMIT, STORAGE_THRESHOLD
...
def add_mega_download(mega_link: str, path: str, listener):
    ...
    api = MegaApi(MEGA_API_KEY, None, None, 'mirrorbot-heroku')
    ...
    if any([STORAGE_THRESHOLD, ZIP_UNZIP_LIMIT, TAR_UNZIP_LIMIT, MEGA_LIMIT]):
        size = api.getSize(node)
        arch = any([listener.isZip, listener.isTar, listener.extract])
        ...
        if ZIP_UNZIP_LIMIT is not None and arch:
            ...
            elif TAR_UNZIP_LIMIT is not None:
            msg3 = f'Failed, Tar limit is {TAR_UNZIP_LIMIT}GB.\nYour File/Folder size is {get_readable_file_size(api.getSize(node))}.'
