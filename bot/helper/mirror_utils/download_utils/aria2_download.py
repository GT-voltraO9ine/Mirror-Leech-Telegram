...
from bot import aria2, download_dict_lock, download_dict, STOP_DUPLICATE, TORRENT_DIRECT_LIMIT, ZIP_UNZIP_LIMIT, TAR_UNZIP_LIMIT, LOGGER, STORAGE_THRESHOLD
...
def __onDownloadStarted(api, gid):
    try:
        if any([STOP_DUPLICATE, TORRENT_DIRECT_LIMIT, ZIP_UNZIP_LIMIT, TAR_UNZIP_LIMIT, STORAGE_THRESHOLD]):
            sleep(1.5)
            ...
            if any([ZIP_UNZIP_LIMIT, TAR_UNZIP_LIMIT, TORRENT_DIRECT_LIMIT, STORAGE_THRESHOLD]):
                sleep(1)
                limit = None
                size = api.get_download(gid).total_length
                arch = any([dl.getListener().isZip, dl.getListener().isTar, dl.getListener().extract])
                ...
                if ZIP_UNZIP_LIMIT is not None and arch:
                    ...
                    elif TAR_UNZIP_LIMIT is not None and arch:
