...
from bot import Interval, INDEX_URL, BUTTON_FOUR_NAME, BUTTON_FOUR_URL, BUTTON_FIVE_NAME, BUTTON_FIVE_URL, \
                ...
                dispatcher, DOWNLOAD_DIR, download_dict, download_dict_lock, TAR_UNZIP_LIMIT, TG_SPLIT_SIZE, LOGGER
...
class MirrorListener:
    def __init__(self, bot, message, isTar=False, isZip=False, extract=False, isQbit=False, isLeech=False, pswd=None, tag=None):
        self.bot = bot
    ...
    def onDownloadComplete(self):
        ...
        if self.isTar:
            try:
                ...
                path = m_path + ".tar"
                LOGGER.info(f'Tar: orig_path: {m_path}, tar_path: {path}')
            ...
            try:
                ...
                if ospath.isdir(m_path):
                        ...
                        for file_ in files:
                            if file_.endswith(".zip") or search(r'\.part0*1\.rar$|\.7z\.0*1$|\.zip\.0*1$', file_) \
                               or (file_.endswith(".rar") and not search(r'\.part\d+\.rar$', file_)):
                                ...
                                if file_.endswith(".tar") or search(r'\.part0*1\.rar$|\.7z\.0*1$|\.tar\.0*1$', file_) \
                               or (file_.endswith(".rar") and not search(r'\.part\d+\.rar$', file_)):
                                m_path = ospath.join(dirpath, file_)
                                if self.pswd is not None:
                    ...
                    path = f'{DOWNLOAD_DIR}{self.uid}/{name}'
                            if file_.endswith((".rar", ".tar")) or search(r'\.r\d+$|\.7z\.\d+$|\.z\d+$|\.tar\.\d+$', file_):
                                del_path = ospath.join(dirpath, file_)
                                osremove(del_path)
                            else:
                    if self.pswd is not None:
...
def _mirror(bot, message, isZip=False, istar=False, extract=False, isQbit=False, isLeech=False, pswd=None):
    ...
    if reply_to is not None:
        ...
        if (
            ...
            if file is None:
                ...
                listener = MirrorListener(bot, message, isZip, isTar, extract, isQbit, isLeech, pswd, tag)
    ...
    if is_gdrive_link(link):
        ...
        if not isTar and not extract and not isLeech:
            gmsg = f"Use /{BotCommands.CloneCommand} to clone Google Drive file/folder\n\n"
            gmsg += f"Use /{BotCommands.TarMirrorCommand} to make tar of Google Drive folder\n\n"
            gmsg += f"Use /{BotCommands.UnzipMirrorCommand} to extracts Google Drive archive file"
            return sendMessage(gmsg, bot, message)
        Thread(target=add_gd_download, args=(link, listener, is_gdtot)).start()
